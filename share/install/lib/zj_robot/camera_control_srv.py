...
def camera_realplay_process_func(camera_cmd,img_pub):
    ...
    while True:
        ...
        if camera_cmd.value!=0:
            ...
            size = (int(cap.cap.get(cv2.CAP_PROP_FRAME_WIDTH)),int(cap.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))

            time_now=str(datetime.now()).replace(" ","")
            out = cv2.VideoWriter('/home/zj/script/video/video_'+time_now+'.avi',fourcc ,fps, size)
            num=0
